...
\begin{figure}[htbp]
\begin{eqnarray*}
\ntDef{VariableDeclaration}&\arrow&\q{var}\,\ntRef{Pattern}\ \q{is}\ \ntRef{Expression}\\
&\choice&\ntRef{Identifier}\ \q{is}\ \ntRef{Expression}\\
&\choice&\q{var}\ \ntRef{Identifier}\ \q{:=}\ \ntRef{Expression}
\end{eqnarray*}
\caption{Variable Declaration}\label{variableDeclarationFig}
\end{figure}

The \q{var} keyword is mandatory when declaring re-assignable variables and optional -- in the case that a single variable is declared -- when declaring single-assignment variables.
...
ref \emph{type}
\end{alltt}
rather than simply \q{\emph{type}}. For example, given the declaration:
\begin{alltt}
var Ix := 0
\end{alltt}
