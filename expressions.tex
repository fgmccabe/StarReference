...
\item In a \q{has value} condition:
\q{L[Ix] has value V}
...
\end{alltt}
\begin{aside}
The type of the resulting value from an indexed access is \q{option}al. I.e., since it cannot be guaranteed -- in general -- that the element exists, the resulting type expresses that uncertainty.

You can extract the underlying value by using the \q{someValue} function -- see Section~\vref{someValue} -- but a safer way is to use a \q{has value} condition (see Section~\vref{optionUnwrap}.
\end{aside}
