...
\begin{alltt}
\pcent{}\pcent{}c of (\pcent{}t\sub1\sequence{,}\pcent{}t\subn)
\end{alltt}
...
is equivalent to:
\begin{alltt}
\pcent{}c of (\pcent{}t\sub1\sequence{,}\pcent{}t\subn) where \pcent{}c has kind type of (type\sequence{,}type)
\end{alltt}
A \q{\pcent{}\pcent{}c} appearing on its own is assumed to have arity 1:
\begin{alltt}
...
\end{alltt}
A type variable \q{\pcent{}s} may be bound to a type expression such as \q{cons of string}, but not to the type constructor \q{cons}.

Conversely, a type variable \q{\pcent{}\pcent{}c} may be bound to the \q{cons} type constructor, but may \emph{not} be bound to a type expression such as \q{cons of string}.
\begin{aside}
The \q{\pcent{}\pcent{}} form of type variable is equivalent to an implied \ntRef{HasKind} constraint -- see Section~\vref{hasKindConstraint}: the type variable \q{\pcent{}\pcent{}c} is equivalent to:
...
R.f1.f2.t
\end{alltt}
\item The `value' of an encapsulated type is strictly opaque: it is guaranteed to be different to all other types. Which means that effectively \emph{only} the other fields of the record variable \q{R} contain functions and values that can be used in conjunction.
\end{itemize}
...
\begin{aside}
\ntRef{HasKindConstraint}s typically arise from special forms of type expressions. For example, as noted in Section~\vref{typeVariable}, a type expression of the form:
\begin{alltt}
\pcent{}\pcent{}e
\end{alltt}
